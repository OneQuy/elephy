...
import { ContactType, DeveloperNote, RemoteConfig, UserProperty_StartUsingAppTick, ExtendNotificationTrackData, RapidApiLimit } from "./SpecificType"
import { GetArrayAsync_PickAndRemoveFirstOne } from "./AsyncStorageUtils"
import { StorageKey_CacheEventNotification, StorageKey_StartUsingAppTick } from "../App/Constants/StorageKey"
import { TrackEventNotificationAsync, TrackPress, TrackSimpleWithParam } from "./Tracking"
...
/**
 * ### note: remaining could be negative.
 */
export function GetRapidApiLimit(respone: XMLHttpRequest): undefined | RapidApiLimit {
    const limit = parseInt(respone.getResponseHeader('x-ratelimit-requests-limit') ?? '')
    const remaining = parseInt(respone.getResponseHeader('x-ratelimit-requests-remaining') ?? '')
